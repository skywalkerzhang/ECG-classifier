...
def training_code(data_directory = 'training_data', model_directory = 'model'):
    ...
    # 针对不同个数的leads，把需要的Leads抽出来
    feature_indices = [twelve_leads.index(lead) for lead in leads] + [12, 13]
    features = data[:, feature_indices]

    # 缺失值填充
    # 要自己实现对数据的预处理，性别和年纪有空值
    # features[0] 代表ECG图像数据 features[1]代表年纪 features[2]代表性别
